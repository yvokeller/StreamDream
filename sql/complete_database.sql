...
CREATE DEFINER=`root`@`localhost` PROCEDURE `select_episodes_by_season_and_series` (IN `d_id_series` INT(11), IN `d_id_season` INT(11))  BEGIN



SELECT e.id, e.name, e.description, e.thumbnail, e.src, seaepi.number
FROM tbl_episode e
...
WHERE ser.id = d_id_series AND sea.id = d_id_season
...
SELECT e.id, e.name, e.description, e.thumbnail, e.src, e.year production_year, seaepi.number, sea.name name_season, ser.name name_series, ser.description desc_series, sea.order_number
FROM tbl_episode e
INNER JOIN tbl_season_episode seaepi ON e.id = seaepi.fk_episode
INNER JOIN tbl_season sea ON seaepi.fk_season = sea.id
INNER JOIN tbl_series ser ON sea.fk_series = ser.id
